...
async def chat(request: ChatRequest):
    ...
    def generate():
        ...
        async def stream():
            async for chunk in orchestrator.run_stream(request.message):
                yield chunk
        return stream()
...
async def chat_mistral(request: ChatRequest):
    def generate():
        for chunk in nvidia_service.chat_mistral_large(request.message, request.history):
            yield chunk
    return StreamingResponse(generate(), media_type="text/plain")
...
async def chat_mixtral(request: ChatRequest):
    def generate():
        for chunk in nvidia_service.chat_mixtral_8x22b(request.message, request.history):
            yield chunk
    return StreamingResponse(generate(), media_type="text/plain")
...
async def chat_glm5(request: ChatRequest):
    def generate():
        for chunk in nvidia_service.chat_glm5(request.message, request.history):
